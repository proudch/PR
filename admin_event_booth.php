...
//ถ้ามีการส่งค่าข้อมูล
                            ...
                            while ($data = mysqli_fetch_array($query) ) {?>
                                ...
                                <h4 class="title">โซน:&nbsp;&nbsp;&nbsp;&nbsp;<?php echo "$data[name]"; ?></h4>
                                <?php
                            }
                            ?>
                        </div>
                        ...
                        $sql = "SELECT * FROM `booths` WHERE zone_id = '".$zone_id."' ";
                        $query = mysqli_query($con,$sql);
                        while ($result = mysqli_fetch_array($query) ) { ?>
                            <tr>
                                ...
                                <?php 
                                if ($result["status"] =="f") {
                                    echo "<td><p class='text-danger'> ว่าง
                                    </td>.";

                                     echo "<td><a href='order.php?booth_id=$result[booth_id]' class='btn btn-success'>จอง</a></td>";
                                    echo "<td><a href='admin_event_booth.php?zone_id=<?php echo $result[booth_id];?>' class='btn btn-danger'>ยกเลิก</a></td>";
                                    echo "<td><input type='hidden' name='booth_id' value='$result[booth_id]'></td>";
                                }
                                elseif ($result["status"] =="w") {
                                 echo "<td><p class='text-warning'> รอชำระเงิน
                                 </td>";
                                 echo "<td><input type='hidden' name='booth_id' value='$result[booth_id]'></td>";
                                 echo "<td><a href='admin_event_booth.php?zone_id=<?php echo $data[zone_id];?>' class='btn btn-danger'>ยกเลิก</a></td>";
                               
                             }
                             else {
                                 echo "<td><p class='text-success'> ชำระเงินแล้ว
                                 </td>.";
                               echo "<td><input type='hidden' name='booth_id' value='$result[booth_id]'></td>";
                             }   
